...
import type { Game } from './GameBadge';

interface GameCardProps {
  game: Game;
  onInstall?: (g: Game) => void;
  onDelete?: (g: Game) => void;
}

export function GameCard({ game, onInstall, onDelete }: GameCardProps) {
  const fmt = gameFormatMeta(game.format || 'html');
  ...
  const installed = (game.install_status || 'installed') !== 'disabled';
    ...
    >
      <div className="flex items-start gap-3">
        <div
          ...
          style={{ background: `${fmt.color}1a`, border: `1px solid ${fmt.color}40` }}
        >
          {game.icon || '🎮'}
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <h4 className="font-medium text-sm truncate">{game.name || game.id}</h4>
          ...
          <div className="mt-1 flex items-center gap-2 font-mono text-[9px] text-muted-foreground">
            <span className="inline-flex items-center gap-1">
              <Icon name="Users" size={9} /> {players}
            </span>
            <span className="inline-flex items-center gap-1">
              <Icon name="Grid3x3" size={9} /> {game.grid_size || 8}×{game.grid_size || 8}
            </span>
          </div>
        </div>
        <span
      ...
      <div className="mt-3 space-y-1.5">
        <Tier
          ...
          color="#34d399"
          hint="inherent"
          tools={SYSTEM_MANDATORY_CAPABILITIES.map((c) => c.name)}
          locked
        />
        {mandatoryTools.length > 0 && (
          <Tier
            ...
            icon="Star"
            color="#fbbf24"
            hint="required to play"
            tools={mandatoryTools.map((t) => t.name)}
            locked
          />
        )}
        {specialTools.length > 0 && (
          <Tier
            ...
            icon="Circle"
            color="#a78bfa"
            hint="toggleable per agent"
            tools={specialTools.map((t) => t.name)}
          />
        )}
      </div>
      ...
      <div className="mt-3 flex items-center gap-1.5 flex-wrap">
        <span
          ...
          style={{ color: fmt.color, background: `${fmt.color}14` }}
        >
    ...
    <div className="rounded-lg border border-border bg-card/30 px-2 py-1.5">
      ...
      <div className="flex flex-wrap gap-1">
        ...
        {tools.map((t, i) => (
          <span
            key={i}
            className="rounded bg-muted/50 px-1.5 py-0.5 font-mono text-[9px] text-muted-foreground"
          >
            {t}
          </span>
        ))}
