...
import { Controller } from '@ai-game-arena/controllers';
import type { GameBridge } from '@ai-game-arena/platforms';
import { AgentRuntime } from '@ai-game-arena/ai-runtime';
...
export class MatchEngine {
  ...
  private adapterFactory?: (gameId?: string) => GameBridge | null;
  ...
  async start(): Promise<void> {
    ...
    for (const agent of this.agents) {
      ...
      // Not every bridge exposes tools (e.g. raw platform bridges) — optional.
      if (typeof bridge?.registerTools === 'function') {
  ...
  private createBridge(): GameBridge | null {
    ...
    return this.adapterFactory(this.gameId) ?? null;
  }
